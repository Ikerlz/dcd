...
if __name__ == '__main__':
    data = loadmat('data/icml2010.mat')
    agblog_adj_matrix = data['agblog'].A
    for i in range(1222):
        agblog_adj_matrix[i][i] = 10
    index_list = [x for x in range(1222)]
    agblog_data_label = np.squeeze(data['agblog_label']).tolist()
    agblog_data_label = [(x - 1) for x in agblog_data_label]
        ...
        for repeat in range(repeat_number):
            mis_rate_i, running_time_i = \
                spark_pilot_spectral_clustering(total_size, pilot_node_number,
                                                ...
                                                worker_per_sub, agblog_adj_matrix, index2label_dict)
    ...
    #                                  real_data=True)
    #     pilot_node_number = math.ceil(pilot_ratio * total_size)
    #     master_pdf, worker_pdf = split_master_worker(agblog_adj_matrix, index2label_dict,
